# Template Specification: Khatim

## 1. TEMPLATE OVERVIEW
- **Template Name:** Khatim
- **Template ID:** `TPL-ISL-01`
- **Slug:** `khatim`
- **Category:** Islamic
- **Religious / Cultural Category:** Classic Arabesque / Traditional Nikah
- **Price / Tier:** ₹1,199
- **Reference URL:** [https://saveourdate.in/templates/khatim](https://saveourdate.in/templates/khatim)
- **Live Demo URL:** [https://saveourdate.in/templates/khatim/live](https://saveourdate.in/templates/khatim/live)

### Design Language & Visual Direction
- **Visual Direction:** 8-Pointed Islamic Star Geometry / Sacred Mihrab Arches / Emerald & Antique Gold
- **Primary Mood:** Sacred / Spiritual / Timeless / Harmonious
- **Visual Density:** High (Khatim star lattice, gilded muqarnas arch borders, Quranic calligraphy accents)
- **Animation Intensity:** High (Rotating 8-pointed Khatim star expansion, arched Mihrab door split, shimmering golden light rays)
- **Color Palette Breakdown:**
  - Primary Background: `#F9F7F1`
  - Secondary Tone: `#004D40`
  - Accent Color: `#D4AF37`
  - Dark Accent: `#00332C`
  - Neutral Dark / Text: `#1A2421`
  - Card Glass Surface: `rgba(249, 247, 241, 0.96)`
- **Typography Matrix:**
  - Display Headline: `Cinzel, serif`
  - Subheadings: `Cormorant Garamond, serif`
  - Body Text: `Tajawal / Montserrat, sans-serif`
  - Accent Calligraphy: `Scheherazade New, serif`

---

## 2. COMPLETE USER EXPERIENCE
The end-to-end guest journey for **Khatim** follows this sequence:

1. **Initial Page Load:** Full-viewport branded staging layer with delicate background motif and pulsing hint.
2. **Opening Screen Presentation:** The custom **Geometric 8-Pointed Khatim Star Gate Expanding with Bismillah Calligraphy Fade** covers the entire screen, preventing accidental content leak while awaiting user gesture.
...
6. **Smooth Downward Scroll:** Guest scrolls down at their own pace; floating decorative elements (Subtle Floating Gold Dust, Crescent Star Geometry & Jasmine Petals) subtly drift across the screen.
...
- **Loading Screen Visuals:** Minimalist full-screen container colored in `#F9F7F1` with a subtle radial gradient glow (`#D4AF37` at 15% opacity).
...
- **Opening Mechanism:** Geometric 8-Pointed Khatim Star Gate Expanding with Bismillah Calligraphy Fade
- **Initial Visual State:**
  - Split panels or layered envelope flaps positioned over the viewport at `z-index: 50`.
  - Intricate decorative borders matching the **Khatim** visual aesthetic (`#D4AF37` metallic filigree).
...
- **Active Decorative Elements:** Subtle Floating Gold Dust, Crescent Star Geometry & Jasmine Petals
...
- **Content Hierarchy:**
  1. **Top Eyebrow:** Auspicious blessing / Tagline in uppercase tracking `0.25em` (`Cormorant Garamond, serif`).
  2. **Couple Monogram / Emblem:** Centered crest featuring stylized initials.
  3. **Bride & Groom Names:** Large serif display typography (`Cinzel, serif`), colored in `#00332C` or `#004D40`, separated by an artistic ampersand (`&`) or infinity knot.
...
## 15. VENUE / MAP
- **Map Integration:** Embedded Google Maps container styled with custom warm stone/monochrome map skin matching `#F9F7F1`.
...
- `<KhatimOpening />` (Custom Geometric 8-Pointed Khatim Star Gate Expanding with Bismillah Calligraphy Fade)
- `<KhatimHero />`
- `<KhatimDecorations />`
...
```typescript
export const khatim_Config = {
  templateId: "TPL-ISL-01",
  slug: "khatim",
  name: "Khatim",
  category: "Islamic",
  theme: {
    colors: {
      ...
      darkAccent: "#00332C",
      neutralDark: "#1A2421",
      cardBg: "rgba(249, 247, 241, 0.96)",
    },
    typography: {
      display: "Cinzel, serif",
      subheading: "Cormorant Garamond, serif",
      body: "Tajawal / Montserrat, sans-serif",
  ...
  decorations: {
    floatingElements: "Subtle Floating Gold Dust, Crescent Star Geometry & Jasmine Petals",
...
## 28. ASSET INVENTORY
- **Decorative SVGs:**
  - Arch frames: `/templates/khatim/assets/arch-frame.svg`
  - Floral corner motifs: `/templates/khatim/assets/corner-flourish.svg`
  - Seal / Monogram frame: `/templates/khatim/assets/seal-crest.svg`
  - Divider flourishes: `/templates/khatim/assets/divider.svg`
- **Audio Assets:**
  - Ambient soundtrack: `/templates/khatim/audio/ambient-track.mp3`
...
- `01-initial-gate.png`: Initial locked viewport state with "Geometric 8-Pointed Khatim Star Gate Expanding with Bismillah Calligraphy Fade" prompt.
...
- **Recommended Opening Component:** `<KhatimOpening />` using Framer Motion spring orchestration.
- **Recommended Hero Component:** `<KhatimHero />` with staggered serif text reveals.
...
- **Template Architecture:** Driven purely by normalized `WeddingData` and template configuration (`khatim_Config`).
