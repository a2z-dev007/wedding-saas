# Template Specification: Zaytoon

## 1. TEMPLATE OVERVIEW
- **Template Name:** Zaytoon
- **Template ID:** `TPL-ISL-03`
- **Slug:** `zaytoon`
- **Category:** Islamic
- **Religious / Cultural Category:** Mediterranean Olive Grove / Modern Nikah
- **Price / Tier:** ₹1,099
- **Reference URL:** [https://saveourdate.in/templates/zaytoon](https://saveourdate.in/templates/zaytoon)
- **Live Demo URL:** [https://saveourdate.in/templates/zaytoon/live](https://saveourdate.in/templates/zaytoon/live)

### Design Language & Visual Direction
- **Visual Direction:** Olive Branch Watercolors / Warm Sandstone / Minimalist Terrazzo & Arabic Script
- **Primary Mood:** Peaceful / Earthy / Blessed / Modern Minimalist
- **Visual Density:** Medium-Low (Warm stone neutrals, delicate olive sprigs, clean typography)
- **Animation Intensity:** Medium (Linen scroll unroll, gently swaying olive leaves, soft golden sunlight fade)
- **Color Palette Breakdown:**
  - Primary Background: `#FAF8F5`
  - Secondary Tone: `#556B2F`
  - Accent Color: `#C2A649`
  - Dark Accent: `#3B4D1F`
  - Neutral Dark / Text: `#22261E`
  - Card Glass Surface: `rgba(250, 248, 245, 0.92)`
- **Typography Matrix:**
...
The end-to-end guest journey for **Zaytoon** follows this sequence:

1. **Initial Page Load:** Full-viewport branded staging layer with delicate background motif and pulsing hint.
2. **Opening Screen Presentation:** The custom **Linen Parchment Scroll with Olive Branch Seal Unrolling Vertically** covers the entire screen, preventing accidental content leak while awaiting user gesture.
...
6. **Smooth Downward Scroll:** Guest scrolls down at their own pace; floating decorative elements (Falling Olive Leaves, Jasmine Buds & Warm Ambient Sun Dust) subtly drift across the screen.
...
- **Loading Screen Visuals:** Minimalist full-screen container colored in `#FAF8F5` with a subtle radial gradient glow (`#C2A649` at 15% opacity).
...
- **Opening Mechanism:** Linen Parchment Scroll with Olive Branch Seal Unrolling Vertically
- **Initial Visual State:**
  - Split panels or layered envelope flaps positioned over the viewport at `z-index: 50`.
  - Intricate decorative borders matching the **Zaytoon** visual aesthetic (`#C2A649` metallic filigree).
...
- **Active Decorative Elements:** Falling Olive Leaves, Jasmine Buds & Warm Ambient Sun Dust
...
- **Content Hierarchy:**
  ...
  3. **Bride & Groom Names:** Large serif display typography (`Cormorant Garamond, serif`), colored in `#3B4D1F` or `#556B2F`, separated by an artistic ampersand (`&`) or infinity knot.
...
- **Form Inputs:** Active floating labels and shimmering gold outline focus rings (`ring-2 ring-[#C2A649]`).
...
- `<ZaytoonOpening />` (Custom Linen Parchment Scroll with Olive Branch Seal Unrolling Vertically)
- `<ZaytoonHero />`
- `<ZaytoonDecorations />`
...
```typescript
export const zaytoon_Config = {
  templateId: "TPL-ISL-03",
  slug: "zaytoon",
  name: "Zaytoon",
  category: "Islamic",
  theme: {
  ...
  decorations: {
    floatingElements: "Falling Olive Leaves, Jasmine Buds & Warm Ambient Sun Dust",
...
## 28. ASSET INVENTORY
- **Decorative SVGs:**
  - Arch frames: `/templates/zaytoon/assets/arch-frame.svg`
  - Floral corner motifs: `/templates/zaytoon/assets/corner-flourish.svg`
  - Seal / Monogram frame: `/templates/zaytoon/assets/seal-crest.svg`
  - Divider flourishes: `/templates/zaytoon/assets/divider.svg`
- **Audio Assets:**
  - Ambient soundtrack: `/templates/zaytoon/audio/ambient-track.mp3`
...
- `01-initial-gate.png`: Initial locked viewport state with "Linen Parchment Scroll with Olive Branch Seal Unrolling Vertically" prompt.
...
- **Recommended Opening Component:** `<ZaytoonOpening />` using Framer Motion spring orchestration.
- **Recommended Hero Component:** `<ZaytoonHero />` with staggered serif text reveals.
...
- **Template Architecture:** Driven purely by normalized `WeddingData` and template configuration (`zaytoon_Config`).
