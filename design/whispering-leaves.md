# Template Specification: Whispering Leaves

## 1. TEMPLATE OVERVIEW
- **Template Name:** Whispering Leaves
- **Template ID:** `TPL-NON-01`
- **Slug:** `whispering-leaves`
- **Category:** Non-Religious
- **Religious / Cultural Category:** Modern Botanical / Fine Art Minimalist
- **Price / Tier:** ₹999
- **Reference URL:** [https://saveourdate.in/templates/whispering-leaves](https://saveourdate.in/templates/whispering-leaves)
- **Live Demo URL:** [https://saveourdate.in/templates/whispering-leaves/live](https://saveourdate.in/templates/whispering-leaves/live)

### Design Language & Visual Direction
- **Visual Direction:** Sage Green Botanical / Eucalyptus Watercolor / Fine Art Editorial
- **Primary Mood:** Serene / Organic / Romantic / Modern Minimalist
- **Visual Density:** Medium-Low (Airy spacing, watercolor foliage, delicate hairlines)
- **Animation Intensity:** Medium (Soft parchment unroll, drifting eucalyptus leaves, subtle parallax layers)
- **Color Palette Breakdown:**
  - Primary Background: `#F7F5F0`
  - Secondary Tone: `#4F5D48`
  - Accent Color: `#8A9A86`
  - Dark Accent: `#2F3E2E`
  - Neutral Dark / Text: `#222621`
  - Card Glass Surface: `rgba(247, 245, 240, 0.9)`
- **Typography Matrix:**
  - Display Headline: `Cormorant Garamond, serif`
  - Subheadings: `Newsreader, serif`
  - Body Text: `Inter / Geist Sans, sans-serif`
  - Accent Calligraphy: `Pinyon Script, cursive`

---

## 2. COMPLETE USER EXPERIENCE
The end-to-end guest journey for **Whispering Leaves** follows this sequence:

1. **Initial Page Load:** Full-viewport branded staging layer with delicate background motif and pulsing hint.
2. **Opening Screen Presentation:** The custom **Textured Linen Card with Floating Botanical Leaf Reveal & Audio Unlock** covers the entire screen, preventing accidental content leak while awaiting user gesture.
...
6. **Smooth Downward Scroll:** Guest scrolls down at their own pace; floating decorative elements (Drifting Eucalyptus Leaves & Gentle Ambient Wind Sway) subtly drift across the screen.
...
- **Loading Screen Visuals:** Minimalist full-screen container colored in `#F7F5F0` with a subtle radial gradient glow (`#8A9A86` at 15% opacity).
...
- **Opening Mechanism:** Textured Linen Card with Floating Botanical Leaf Reveal & Audio Unlock
- **Initial Visual State:**
  - Split panels or layered envelope flaps positioned over the viewport at `z-index: 50`.
  - Intricate decorative borders matching the **Whispering Leaves** visual aesthetic (`#8A9A86` metallic filigree).
...
- **Active Decorative Elements:** Drifting Eucalyptus Leaves & Gentle Ambient Wind Sway
...
- **Content Hierarchy:**
  1. **Top Eyebrow:** Auspicious blessing / Tagline in uppercase tracking `0.25em` (`Newsreader, serif`).
  2. **Couple Monogram / Emblem:** Centered crest featuring stylized initials.
  3. **Bride & Groom Names:** Large serif display typography (`Cormorant Garamond, serif`), colored in `#2F3E2E` or `#4F5D48`, separated by an artistic ampersand (`&`) or infinity knot.
...
## 15. VENUE / MAP
- **Map Integration:** Embedded Google Maps container styled with custom warm stone/monochrome map skin matching `#F7F5F0`.
...
- **Form Inputs:** Active floating labels and shimmering gold outline focus rings (`ring-2 ring-[#8A9A86]`).
...
- `<WhisperingLeavesOpening />` (Custom Textured Linen Card with Floating Botanical Leaf Reveal & Audio Unlock)
- `<WhisperingLeavesHero />`
- `<WhisperingLeavesDecorations />`
...
```typescript
export const whispering_leaves_Config = {
  templateId: "TPL-NON-01",
  slug: "whispering-leaves",
  ...
  theme: {
    colors: {
      ...
      darkAccent: "#2F3E2E",
      neutralDark: "#222621",
      cardBg: "rgba(247, 245, 240, 0.9)",
    },
    typography: {
      display: "Cormorant Garamond, serif",
      subheading: "Newsreader, serif",
      body: "Inter / Geist Sans, sans-serif",
      accentScript: "Pinyon Script, cursive",
    },
  },
  opening: {
    type: "curtain-reveal",
  ...
  decorations: {
    floatingElements: "Drifting Eucalyptus Leaves & Gentle Ambient Wind Sway",
...
## 28. ASSET INVENTORY
- **Decorative SVGs:**
  - Arch frames: `/templates/whispering-leaves/assets/arch-frame.svg`
  - Floral corner motifs: `/templates/whispering-leaves/assets/corner-flourish.svg`
  - Seal / Monogram frame: `/templates/whispering-leaves/assets/seal-crest.svg`
  - Divider flourishes: `/templates/whispering-leaves/assets/divider.svg`
- **Audio Assets:**
  - Ambient soundtrack: `/templates/whispering-leaves/audio/ambient-track.mp3`
...
- `01-initial-gate.png`: Initial locked viewport state with "Textured Linen Card with Floating Botanical Leaf Reveal & Audio Unlock" prompt.
...
- **Recommended Opening Component:** `<WhisperingLeavesOpening />` using Framer Motion spring orchestration.
- **Recommended Hero Component:** `<WhisperingLeavesHero />` with staggered serif text reveals.
...
- **Template Architecture:** Driven purely by normalized `WeddingData` and template configuration (`whispering_leaves_Config`).
