# Template Specification: Royal Blush

## 1. TEMPLATE OVERVIEW
- **Template Name:** Royal Blush
- **Template ID:** `TPL-NON-02`
- **Slug:** `royal-blush`
- **Category:** Non-Religious
- **Religious / Cultural Category:** Pastel Romance / Rose Gold Luxury
- **Price / Tier:** ₹1,199
- **Reference URL:** [https://saveourdate.in/templates/royal-blush](https://saveourdate.in/templates/royal-blush)
- **Live Demo URL:** [https://saveourdate.in/templates/royal-blush/live](https://saveourdate.in/templates/royal-blush/live)

### Design Language & Visual Direction
- **Visual Direction:** Dusty Rose & Champagne Gold / Soft Petal Bloom / Dreamy Glassmorphism
- **Primary Mood:** Romantic / Delicate / Luxurious / Contemporary
- **Visual Density:** Medium (Soft radial gradients, frosted glass cards, rose gold borders)
- **Animation Intensity:** Medium-High (Rose gold silk ribbon untie, blooming cherry blossoms, soft glow pulses)
- **Color Palette Breakdown:**
  - Primary Background: `#FAF5F0`
  - Secondary Tone: `#B76E79`
  - Accent Color: `#F4D0D9`
  - Dark Accent: `#6E3A44`
  - Neutral Dark / Text: `#2C2224`
  - Card Glass Surface: `rgba(250, 245, 240, 0.88)`
- **Typography Matrix:**
  - Display Headline: `Cormorant Garamond Italic, serif`
  - Subheadings: `Playfair Display, serif`
  - Body Text: `Montserrat, sans-serif`
  - Accent Calligraphy: `Parisienne, cursive`

---

## 2. COMPLETE USER EXPERIENCE
The end-to-end guest journey for **Royal Blush** follows this sequence:

1. **Initial Page Load:** Full-viewport branded staging layer with delicate background motif and pulsing hint.
2. **Opening Screen Presentation:** The custom **Rose Gold Shimmer Envelope with Magnetic Ribbon Pull** covers the entire screen, preventing accidental content leak while awaiting user gesture.
...
6. **Smooth Downward Scroll:** Guest scrolls down at their own pace; floating decorative elements (Falling Pink Cherry Blossom Petals & Soft Rose Gold Glitter) subtly drift across the screen.
...
- **Loading Screen Visuals:** Minimalist full-screen container colored in `#FAF5F0` with a subtle radial gradient glow (`#F4D0D9` at 15% opacity).
...
- **Opening Mechanism:** Rose Gold Shimmer Envelope with Magnetic Ribbon Pull
- **Initial Visual State:**
  - Split panels or layered envelope flaps positioned over the viewport at `z-index: 50`.
  - Intricate decorative borders matching the **Royal Blush** visual aesthetic (`#F4D0D9` metallic filigree).
...
- **Active Decorative Elements:** Falling Pink Cherry Blossom Petals & Soft Rose Gold Glitter
...
- **Content Hierarchy:**
  ...
  3. **Bride & Groom Names:** Large serif display typography (`Cormorant Garamond Italic, serif`), colored in `#6E3A44` or `#B76E79`, separated by an artistic ampersand (`&`) or infinity knot.
...
## 15. VENUE / MAP
- **Map Integration:** Embedded Google Maps container styled with custom warm stone/monochrome map skin matching `#FAF5F0`.
...
- **Form Inputs:** Active floating labels and shimmering gold outline focus rings (`ring-2 ring-[#F4D0D9]`).
...
- `<RoyalBlushOpening />` (Custom Rose Gold Shimmer Envelope with Magnetic Ribbon Pull)
- `<RoyalBlushHero />`
- `<RoyalBlushDecorations />`
...
```typescript
export const royal_blush_Config = {
  templateId: "TPL-NON-02",
  slug: "royal-blush",
  name: "Royal Blush",
  category: "Non-Religious",
  theme: {
  ...
  decorations: {
    floatingElements: "Falling Pink Cherry Blossom Petals & Soft Rose Gold Glitter",
...
## 28. ASSET INVENTORY
- **Decorative SVGs:**
  - Arch frames: `/templates/royal-blush/assets/arch-frame.svg`
  - Floral corner motifs: `/templates/royal-blush/assets/corner-flourish.svg`
  - Seal / Monogram frame: `/templates/royal-blush/assets/seal-crest.svg`
  - Divider flourishes: `/templates/royal-blush/assets/divider.svg`
- **Audio Assets:**
  - Ambient soundtrack: `/templates/royal-blush/audio/ambient-track.mp3`
...
- `01-initial-gate.png`: Initial locked viewport state with "Rose Gold Shimmer Envelope with Magnetic Ribbon Pull" prompt.
...
- **Recommended Opening Component:** `<RoyalBlushOpening />` using Framer Motion spring orchestration.
- **Recommended Hero Component:** `<RoyalBlushHero />` with staggered serif text reveals.
...
- **Template Architecture:** Driven purely by normalized `WeddingData` and template configuration (`royal_blush_Config`).
