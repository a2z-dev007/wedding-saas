# Template Specification: Grand Celebration

## 1. TEMPLATE OVERVIEW
- **Template Name:** Grand Celebration
- **Template ID:** `TPL-HIN-03`
- **Slug:** `grand-celebration`
- **Category:** Hinduism
- **Religious / Cultural Category:** Festive Sangeet / Modern Fusion Bollywood Grandeur
- **Price / Tier:** ₹1,299
- **Reference URL:** [https://saveourdate.in/templates/grand-celebration](https://saveourdate.in/templates/grand-celebration)
- **Live Demo URL:** [https://saveourdate.in/templates/grand-celebration/live](https://saveourdate.in/templates/grand-celebration/live)

### Design Language & Visual Direction
- **Visual Direction:** Festive Marigold / Dazzling Golden Lights / Grand Indian Baarat
- **Primary Mood:** Exuberant / Joyous / Vibrant / Dynamic
- **Visual Density:** High (Fairy lights, marigold garlands, shimmering bokeh lights)
- **Animation Intensity:** High (Grand silk stage curtain rise, bursting confetti & sparkles, beat-pulsing audio player)
- **Color Palette Breakdown:**
  - Primary Background: `#FFF8E7`
  - Secondary Tone: `#FF9933`
  - Accent Color: `#F1C40F`
  - Dark Accent: `#3D0C2A`
  - Neutral Dark / Text: `#1B0513`
  - Card Glass Surface: `rgba(255, 248, 231, 0.95)`
- **Typography Matrix:**
  - Display Headline: `Playfair Display, serif`
  - Subheadings: `Montserrat, sans-serif`
  - Body Text: `Geist Sans, sans-serif`
  - Accent Calligraphy: `Great Vibes, cursive`

---

## 2. COMPLETE USER EXPERIENCE
The end-to-end guest journey for **Grand Celebration** follows this sequence:

1. **Initial Page Load:** Full-viewport branded staging layer with delicate background motif and pulsing hint.
2. **Opening Screen Presentation:** The custom **Grand Royal Silk Drapes Parting with Golden Light Flares & Sangeet Beat** covers the entire screen, preventing accidental content leak while awaiting user gesture.
...
6. **Smooth Downward Scroll:** Guest scrolls down at their own pace; floating decorative elements (Falling Marigold Petals, Golden Shimmer & Confetti Flares) subtly drift across the screen.
...
- **Loading Screen Visuals:** Minimalist full-screen container colored in `#FFF8E7` with a subtle radial gradient glow (`#F1C40F` at 15% opacity).
...
- **Opening Mechanism:** Grand Royal Silk Drapes Parting with Golden Light Flares & Sangeet Beat
- **Initial Visual State:**
  - Split panels or layered envelope flaps positioned over the viewport at `z-index: 50`.
  - Intricate decorative borders matching the **Grand Celebration** visual aesthetic (`#F1C40F` metallic filigree).
...
- **Active Decorative Elements:** Falling Marigold Petals, Golden Shimmer & Confetti Flares
...
- **Content Hierarchy:**
  1. **Top Eyebrow:** Auspicious blessing / Tagline in uppercase tracking `0.25em` (`Montserrat, sans-serif`).
  2. **Couple Monogram / Emblem:** Centered crest featuring stylized initials.
  3. **Bride & Groom Names:** Large serif display typography (`Playfair Display, serif`), colored in `#3D0C2A` or `#FF9933`, separated by an artistic ampersand (`&`) or infinity knot.
...
## 15. VENUE / MAP
- **Map Integration:** Embedded Google Maps container styled with custom warm stone/monochrome map skin matching `#FFF8E7`.
...
- **Form Inputs:** Active floating labels and shimmering gold outline focus rings (`ring-2 ring-[#F1C40F]`).
...
- `<GrandCelebrationOpening />` (Custom Grand Royal Silk Drapes Parting with Golden Light Flares & Sangeet Beat)
- `<GrandCelebrationHero />`
- `<GrandCelebrationDecorations />`
...
```typescript
export const grand_celebration_Config = {
  templateId: "TPL-HIN-03",
  slug: "grand-celebration",
  ...
  theme: {
    colors: {
      ...
      darkAccent: "#3D0C2A",
      neutralDark: "#1B0513",
      cardBg: "rgba(255, 248, 231, 0.95)",
    },
    typography: {
      display: "Playfair Display, serif",
      subheading: "Montserrat, sans-serif",
      body: "Geist Sans, sans-serif",
      accentScript: "Great Vibes, cursive",
    },
  },
  opening: {
    type: "curtain-reveal",
  ...
  decorations: {
    floatingElements: "Falling Marigold Petals, Golden Shimmer & Confetti Flares",
...
## 28. ASSET INVENTORY
- **Decorative SVGs:**
  - Arch frames: `/templates/grand-celebration/assets/arch-frame.svg`
  - Floral corner motifs: `/templates/grand-celebration/assets/corner-flourish.svg`
  - Seal / Monogram frame: `/templates/grand-celebration/assets/seal-crest.svg`
  - Divider flourishes: `/templates/grand-celebration/assets/divider.svg`
- **Audio Assets:**
  - Ambient soundtrack: `/templates/grand-celebration/audio/ambient-track.mp3`
...
- `01-initial-gate.png`: Initial locked viewport state with "Grand Royal Silk Drapes Parting with Golden Light Flares & Sangeet Beat" prompt.
...
- **Recommended Opening Component:** `<GrandCelebrationOpening />` using Framer Motion spring orchestration.
- **Recommended Hero Component:** `<GrandCelebrationHero />` with staggered serif text reveals.
...
- **Template Architecture:** Driven purely by normalized `WeddingData` and template configuration (`grand_celebration_Config`).
