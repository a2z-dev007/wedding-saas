# Template Specification: Azure Dreams

## 1. TEMPLATE OVERVIEW
- **Template Name:** Azure Dreams
- **Template ID:** `TPL-NON-04`
- **Slug:** `azure-dreams`
- **Category:** Non-Religious
- **Religious / Cultural Category:** Coastal Mediterranean / Santorini Destination
- **Price / Tier:** ₹1,099
- **Reference URL:** [https://saveourdate.in/templates/azure-dreams](https://saveourdate.in/templates/azure-dreams)
- **Live Demo URL:** [https://saveourdate.in/templates/azure-dreams/live](https://saveourdate.in/templates/azure-dreams/live)

### Design Language & Visual Direction
- **Visual Direction:** Santorini Azure & White Stucco / Aegean Waves / Coastal Floral Bougainvillea
- **Primary Mood:** Breezy / Fresh / Joyous / Destination Luxury
- **Visual Density:** Medium (Crisp white stucco borders, azure wave separators, magenta bougainvillea)
- **Animation Intensity:** Medium-High (Coastal wave wipe reveal, swaying floral vines, ripple cursor feedback)
- **Color Palette Breakdown:**
  - Primary Background: `#FFFFFF`
  - Secondary Tone: `#1E4E79`
  - Accent Color: `#D4AF37`
  - Dark Accent: `#0072BB`
  - Neutral Dark / Text: `#102A43`
  - Card Glass Surface: `rgba(255, 255, 255, 0.95)`
- **Typography Matrix:**
  - Display Headline: `Cormorant Infant, serif`
  - Subheadings: `Tenor Sans, sans-serif`
...
The end-to-end guest journey for **Azure Dreams** follows this sequence:

1. **Initial Page Load:** Full-viewport branded staging layer with delicate background motif and pulsing hint.
2. **Opening Screen Presentation:** The custom **Santorini Arched Blue Shutters Opening with Ocean Breeze Sound & Audio Unlock** covers the entire screen, preventing accidental content leak while awaiting user gesture.
...
6. **Smooth Downward Scroll:** Guest scrolls down at their own pace; floating decorative elements (Falling Magenta Bougainvillea Petals & Drifting Sun Shimmer) subtly drift across the screen.
...
- **Loading Screen Visuals:** Minimalist full-screen container colored in `#FFFFFF` with a subtle radial gradient glow (`#D4AF37` at 15% opacity).
...
- **Opening Mechanism:** Santorini Arched Blue Shutters Opening with Ocean Breeze Sound & Audio Unlock
- **Initial Visual State:**
  - Split panels or layered envelope flaps positioned over the viewport at `z-index: 50`.
  - Intricate decorative borders matching the **Azure Dreams** visual aesthetic (`#D4AF37` metallic filigree).
...
- **Active Decorative Elements:** Falling Magenta Bougainvillea Petals & Drifting Sun Shimmer
...
- **Content Hierarchy:**
  1. **Top Eyebrow:** Auspicious blessing / Tagline in uppercase tracking `0.25em` (`Tenor Sans, sans-serif`).
  2. **Couple Monogram / Emblem:** Centered crest featuring stylized initials.
  3. **Bride & Groom Names:** Large serif display typography (`Cormorant Infant, serif`), colored in `#0072BB` or `#1E4E79`, separated by an artistic ampersand (`&`) or infinity knot.
...
## 15. VENUE / MAP
- **Map Integration:** Embedded Google Maps container styled with custom warm stone/monochrome map skin matching `#FFFFFF`.
...
- `<AzureDreamsOpening />` (Custom Santorini Arched Blue Shutters Opening with Ocean Breeze Sound & Audio Unlock)
- `<AzureDreamsHero />`
- `<AzureDreamsDecorations />`
...
```typescript
export const azure_dreams_Config = {
  templateId: "TPL-NON-04",
  slug: "azure-dreams",
  name: "Azure Dreams",
  category: "Non-Religious",
  theme: {
  ...
  decorations: {
    floatingElements: "Falling Magenta Bougainvillea Petals & Drifting Sun Shimmer",
...
## 28. ASSET INVENTORY
- **Decorative SVGs:**
  - Arch frames: `/templates/azure-dreams/assets/arch-frame.svg`
  - Floral corner motifs: `/templates/azure-dreams/assets/corner-flourish.svg`
  - Seal / Monogram frame: `/templates/azure-dreams/assets/seal-crest.svg`
  - Divider flourishes: `/templates/azure-dreams/assets/divider.svg`
- **Audio Assets:**
  - Ambient soundtrack: `/templates/azure-dreams/audio/ambient-track.mp3`
...
- `01-initial-gate.png`: Initial locked viewport state with "Santorini Arched Blue Shutters Opening with Ocean Breeze Sound & Audio Unlock" prompt.
...
- **Recommended Opening Component:** `<AzureDreamsOpening />` using Framer Motion spring orchestration.
- **Recommended Hero Component:** `<AzureDreamsHero />` with staggered serif text reveals.
...
- **Template Architecture:** Driven purely by normalized `WeddingData` and template configuration (`azure_dreams_Config`).
