# Template Specification: Reshm-e-Noor

## 1. TEMPLATE OVERVIEW
- **Template Name:** Reshm-e-Noor
- **Template ID:** `TPL-ISL-05`
- **Slug:** `reshm-e-noor`
- **Category:** Islamic
- **Religious / Cultural Category:** Royal Silk Brocade / Velvet & Zari Heritage
- **Price / Tier:** ₹1,299
- **Reference URL:** [https://saveourdate.in/templates/reshm-e-noor](https://saveourdate.in/templates/reshm-e-noor)
- **Live Demo URL:** [https://saveourdate.in/templates/reshm-e-noor/live](https://saveourdate.in/templates/reshm-e-noor/live)

### Design Language & Visual Direction
- **Visual Direction:** Royal Plum Silk (Reshm) / 24K Zari Embroidery / Mughal Tapestry
- **Primary Mood:** Opulent / Aristocratic / Rich Heritage / Celebratory
- **Visual Density:** High (Embroidered zari borders, silk weave textures, scalloped arch medallions)
- **Animation Intensity:** High (Embroidered silk brocade tapestry sliding up, shimmering zari gold threads)
- **Color Palette Breakdown:**
  - Primary Background: `#4A154B`
  - Secondary Tone: `#FDFBF7`
  - Accent Color: `#E5C058`
  - Dark Accent: `#2D0A2E`
  - Neutral Dark / Text: `#1A041B`
  - Card Glass Surface: `rgba(74, 21, 75, 0.95)`
- **Typography Matrix:**
...
The end-to-end guest journey for **Reshm-e-Noor** follows this sequence:

1. **Initial Page Load:** Full-viewport branded staging layer with delicate background motif and pulsing hint.
2. **Opening Screen Presentation:** The custom **Handcrafted Zari Silk Brocade Tapestry Pull with Golden Tassel Interaction** covers the entire screen, preventing accidental content leak while awaiting user gesture.
...
6. **Smooth Downward Scroll:** Guest scrolls down at their own pace; floating decorative elements (Floating Golden Zari Sparkles & Fragrant Velvet Rose Petals) subtly drift across the screen.
...
- **Loading Screen Visuals:** Minimalist full-screen container colored in `#4A154B` with a subtle radial gradient glow (`#E5C058` at 15% opacity).
...
- **Opening Mechanism:** Handcrafted Zari Silk Brocade Tapestry Pull with Golden Tassel Interaction
- **Initial Visual State:**
  - Split panels or layered envelope flaps positioned over the viewport at `z-index: 50`.
  - Intricate decorative borders matching the **Reshm-e-Noor** visual aesthetic (`#E5C058` metallic filigree).
...
- **Active Decorative Elements:** Floating Golden Zari Sparkles & Fragrant Velvet Rose Petals
...
- **Content Hierarchy:**
  ...
  3. **Bride & Groom Names:** Large serif display typography (`Cinzel, serif`), colored in `#2D0A2E` or `#FDFBF7`, separated by an artistic ampersand (`&`) or infinity knot.
...
## 15. VENUE / MAP
- **Map Integration:** Embedded Google Maps container styled with custom warm stone/monochrome map skin matching `#4A154B`.
...
- **Form Inputs:** Active floating labels and shimmering gold outline focus rings (`ring-2 ring-[#E5C058]`).
...
- `<ReshmENoorOpening />` (Custom Handcrafted Zari Silk Brocade Tapestry Pull with Golden Tassel Interaction)
- `<ReshmENoorHero />`
- `<ReshmENoorDecorations />`
...
```typescript
export const reshm_e_noor_Config = {
  templateId: "TPL-ISL-05",
  slug: "reshm-e-noor",
  name: "Reshm-e-Noor",
  category: "Islamic",
  theme: {
  ...
  decorations: {
    floatingElements: "Floating Golden Zari Sparkles & Fragrant Velvet Rose Petals",
...
## 28. ASSET INVENTORY
- **Decorative SVGs:**
  - Arch frames: `/templates/reshm-e-noor/assets/arch-frame.svg`
  - Floral corner motifs: `/templates/reshm-e-noor/assets/corner-flourish.svg`
  - Seal / Monogram frame: `/templates/reshm-e-noor/assets/seal-crest.svg`
  - Divider flourishes: `/templates/reshm-e-noor/assets/divider.svg`
- **Audio Assets:**
  - Ambient soundtrack: `/templates/reshm-e-noor/audio/ambient-track.mp3`
...
- `01-initial-gate.png`: Initial locked viewport state with "Handcrafted Zari Silk Brocade Tapestry Pull with Golden Tassel Interaction" prompt.
...
- **Recommended Opening Component:** `<ReshmENoorOpening />` using Framer Motion spring orchestration.
- **Recommended Hero Component:** `<ReshmENoorHero />` with staggered serif text reveals.
...
- **Template Architecture:** Driven purely by normalized `WeddingData` and template configuration (`reshm_e_noor_Config`).
