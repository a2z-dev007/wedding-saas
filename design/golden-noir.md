# Template Specification: Golden Noir

## 1. TEMPLATE OVERVIEW
- **Template Name:** Golden Noir
- **Template ID:** `TPL-NON-06`
- **Slug:** `golden-noir`
- **Category:** Non-Religious
- **Religious / Cultural Category:** Gatsby Art Deco / 1920s Glamour
- **Price / Tier:** ₹1,199
- **Reference URL:** [https://saveourdate.in/templates/golden-noir](https://saveourdate.in/templates/golden-noir)
- **Live Demo URL:** [https://saveourdate.in/templates/golden-noir/live](https://saveourdate.in/templates/golden-noir/live)

### Design Language & Visual Direction
- **Visual Direction:** Jet Black Obsidian / 24K Gold Geometric Sunbursts / Great Gatsby Art Deco
- **Primary Mood:** Ultra-Luxe / Glamorous / Daring / High Octane
- **Visual Density:** High (Art deco chevron arches, gold foil sunburst rays, polished black glass)
- **Animation Intensity:** High (Art deco geometric fan sunburst expanding outward, metallic gold sheen wipes)
- **Color Palette Breakdown:**
  - Primary Background: `#0A0A0A`
  - Secondary Tone: `#F3E5AB`
  - Accent Color: `#E5A93C`
  - Dark Accent: `#1F1F1F`
  - Neutral Dark / Text: `#000000`
  - Card Glass Surface: `rgba(15, 15, 15, 0.95)`
- **Typography Matrix:**
  - Display Headline: `Poiret One / Market Deco, sans-serif`
  - Subheadings: `Bodoni Moda, serif`
  - Body Text: `Montserrat, sans-serif`
  - Accent Calligraphy: `Italianno, cursive`

---

## 2. COMPLETE USER EXPERIENCE
The end-to-end guest journey for **Golden Noir** follows this sequence:

1. **Initial Page Load:** Full-viewport branded staging layer with delicate background motif and pulsing hint.
2. **Opening Screen Presentation:** The custom **Art Deco Geometric 24K Gold Fan Sunburst Expansion with Tap Interaction** covers the entire screen, preventing accidental content leak while awaiting user gesture.
...
6. **Smooth Downward Scroll:** Guest scrolls down at their own pace; floating decorative elements (Art Deco Gold Geometric Flakes, Champagne Bubbles & Sparkler Flares) subtly drift across the screen.
...
- **Loading Screen Visuals:** Minimalist full-screen container colored in `#0A0A0A` with a subtle radial gradient glow (`#E5A93C` at 15% opacity).
...
- **Opening Mechanism:** Art Deco Geometric 24K Gold Fan Sunburst Expansion with Tap Interaction
- **Initial Visual State:**
  - Split panels or layered envelope flaps positioned over the viewport at `z-index: 50`.
  - Intricate decorative borders matching the **Golden Noir** visual aesthetic (`#E5A93C` metallic filigree).
...
- **Active Decorative Elements:** Art Deco Gold Geometric Flakes, Champagne Bubbles & Sparkler Flares
...
- **Content Hierarchy:**
  1. **Top Eyebrow:** Auspicious blessing / Tagline in uppercase tracking `0.25em` (`Bodoni Moda, serif`).
  2. **Couple Monogram / Emblem:** Centered crest featuring stylized initials.
  3. **Bride & Groom Names:** Large serif display typography (`Poiret One / Market Deco, sans-serif`), colored in `#1F1F1F` or `#F3E5AB`, separated by an artistic ampersand (`&`) or infinity knot.
...
## 15. VENUE / MAP
- **Map Integration:** Embedded Google Maps container styled with custom warm stone/monochrome map skin matching `#0A0A0A`.
...
- **Form Inputs:** Active floating labels and shimmering gold outline focus rings (`ring-2 ring-[#E5A93C]`).
...
- `<GoldenNoirOpening />` (Custom Art Deco Geometric 24K Gold Fan Sunburst Expansion with Tap Interaction)
- `<GoldenNoirHero />`
- `<GoldenNoirDecorations />`
...
```typescript
export const golden_noir_Config = {
  templateId: "TPL-NON-06",
  slug: "golden-noir",
  name: "Golden Noir",
  category: "Non-Religious",
  theme: {
    ...
    typography: {
      display: "Poiret One / Market Deco, sans-serif",
      subheading: "Bodoni Moda, serif",
      body: "Montserrat, sans-serif",
      accentScript: "Italianno, cursive",
    },
  },
  opening: {
    type: "curtain-reveal",
  ...
  decorations: {
    floatingElements: "Art Deco Gold Geometric Flakes, Champagne Bubbles & Sparkler Flares",
...
## 28. ASSET INVENTORY
- **Decorative SVGs:**
  - Arch frames: `/templates/golden-noir/assets/arch-frame.svg`
  - Floral corner motifs: `/templates/golden-noir/assets/corner-flourish.svg`
  - Seal / Monogram frame: `/templates/golden-noir/assets/seal-crest.svg`
  - Divider flourishes: `/templates/golden-noir/assets/divider.svg`
- **Audio Assets:**
  - Ambient soundtrack: `/templates/golden-noir/audio/ambient-track.mp3`
...
- `01-initial-gate.png`: Initial locked viewport state with "Art Deco Geometric 24K Gold Fan Sunburst Expansion with Tap Interaction" prompt.
...
- **Recommended Opening Component:** `<GoldenNoirOpening />` using Framer Motion spring orchestration.
- **Recommended Hero Component:** `<GoldenNoirHero />` with staggered serif text reveals.
...
- **Template Architecture:** Driven purely by normalized `WeddingData` and template configuration (`golden_noir_Config`).
